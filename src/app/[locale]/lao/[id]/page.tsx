...
import {
  ...
  Droplets, Users, CheckCircle2, AlertCircle, Clock, Map, AlertTriangle
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-gradient-to-br from-primary-900 via-primary-800 to-indigo-900 border-b border-primary-800 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 max-w-6xl py-6 relative z-10">
          <div className="flex items-center justify-between mb-5 gap-4">
            <button
              onClick={() => router.back()}
              className="flex items-center text-primary-100 hover:text-white font-medium group transition-colors text-sm bg-white/5 hover:bg-white/10 px-4 py-2 rounded-full w-fit backdrop-blur-sm"
            >
          ...
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
            ...
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-extrabold text-white mb-2 tracking-tight drop-shadow-sm">
                {lao.name}
              </h1>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center mb-4 border-b border-gray-50 pb-3">
                ...
                <h2 className="text-xl font-bold text-gray-900">{t("details.wastewater_system")}</h2>
              </div>
              
              {lao.wastewaterSystems.length > 0 ? (
                <div className="space-y-3">
                  {lao.wastewaterSystems.map(sys => (
                    <div key={sys.id} className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100">
                      ...
                      <p className="text-xs font-medium text-gray-400 mt-3 flex items-center">
            ...
            <section className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center mb-4 border-b border-gray-50 pb-3">
                <div className="w-9 h-9 rounded-full bg-teal-50 flex items-center justify-center mr-3">
                  <Building2 className="w-5 h-5 text-teal-600" />
                </div>
                <h2 className="text-xl font-bold text-gray-900">{t("details.activities")}</h2>
              </div>
              ...
              <div className="space-y-4">
                {lao.activities
                  ...
                  .map(act => (
                    <div key={act.id} className="relative pl-8 before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-0 before:w-0.5 before:bg-teal-100 last:before:hidden">
                      ...
                      <div className="bg-teal-50/30 rounded-2xl p-5 border border-teal-100/50 hover:bg-teal-50/50 transition-colors">
                        <span className="text-xs font-bold text-teal-600 mb-1.5 block">{act.date}</span>
                        <h3 className="font-bold text-lg text-gray-900 mb-1.5">{act.title}</h3>
                ...
                {lao.activities.filter(a => a.category === 'wastewater_management').length === 0 && (
                  <p className="text-gray-500 py-3">{t("details.no_data")}</p>
                )}
              </div>
            </section>
            ...
            <section className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center mb-4 border-b border-gray-50 pb-3">
                ...
                <h2 className="text-xl font-bold text-gray-900">{t("details.participation")}</h2>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {lao.activities
                  ...
                  .map(act => (
                    <div key={act.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group">
                      ...
                      <div className="p-4">
                        <h3 className="font-bold text-gray-900 mb-1.5 leading-tight group-hover:text-orange-600 transition-colors">{act.title}</h3>
                ...
                {lao.activities.filter(a => a.category === 'community_participation').length === 0 && (
                  <p className="text-gray-500 py-3 col-span-full">{t("details.no_data")}</p>
                )}
              </div>
            </section>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 sticky top-24">
              ...
              <div className="space-y-3 mb-4">
                <div>
                  ...
                  <p className="text-gray-600 mt-0.5">ต.{lao.subdistrict} อ.{lao.district}</p>
                  <p className="text-gray-600 mt-0.5">จ.{lao.province} {lao.zipcode}</p>
                </div>
